...
from src.data.cxr14_pytorch import CXR14Dataset
...
def main(
    ...
    test_dataset = CXR14Dataset()
    test_dataloader = DataLoader(test_dataset, batch_size=32)

    with open(PROCESSED_DATA_DIR/f"cxr14_aucs.csv","w") as perf_file:
    ...
    for folder in training_folders:
        ...
        for fold_nb in tqdm(range(5)):
            ...
            pd.DataFrame.from_dict(preds_csv, orient='index').to_csv(f"{folder}/cxr14_predictions_fold{fold_nb}.csv")
         

        with open(PROCESSED_DATA_DIR/f"cxr14_aucs.csv","a+") as perf_file:
