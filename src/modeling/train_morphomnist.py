...
def main(
    ...
    training_folder = INTERIM_DATA_DIR / f"morphomnist_trainings/{dataset_config}_{training_datetime}"
    ...
    for i, (train_index, val_index) in enumerate(skf.split(X, y)):
            ...
            for j in range(len(lst_img_ids)):
                ...
                if sample_id not in datamaps_info:
                    datamaps_info[sample_id] = {
                        ...
                        "label":lst_labels[j],
                        "proba_label":[float(lst_probas[i][int(lst_labels[j])])]
                    }
                else:
        ...
        logger.success(f"Completed epoch {e}")
