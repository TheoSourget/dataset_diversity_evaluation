...
class Thinning(Perturbation):
    ...
    def __call__(self, morph: ImageMorphology) -> np.ndarray:
        ...
        morph.binary_image = morphology.erosion(morph.binary_image, morphology.disk(radius))
...
class Thickening(Perturbation):
    ...
    def __call__(self, morph: ImageMorphology) -> np.ndarray:
        radius = int(self.amount * morph.scale * morph.mean_thickness / 2.)
        morph.binary_image = morphology.dilation(morph.binary_image, morphology.disk(radius))
        return morph.binary_image
...
class Swelling(Deformation):
    ...
    def __init__(self, strength: float = 3, radius: float = 7,random_seed=None):
        """
        Parameters
        ----------
        ...
        self.radius = radius
        self.loc_sampler = skeleton.LocationSampler(random_seed=random_seed)
...
class Fracture(Perturbation):
    ...
    def __init__(self, thickness: float = 1.5, prune: float = 2, num_frac: int = 3, random_seed=None):
        """
        Parameters
        ----------
        ...
        self.loc_sampler = skeleton.LocationSampler(prune, prune,random_seed=random_seed)
    ...
    def __call__(self, morph: ImageMorphology) -> np.ndarray:
        ...
        morph.binary_image = frac_img[r:-r, r:-r]
